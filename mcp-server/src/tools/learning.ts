 ...
 */
const styleResources: Record<string, { preferredTypes: string[]; avoidTypes: string[]; advice: string }> = {
  visual: {
...
export function registerLearningTools(server: McpServer): void {
  server.tool(
    ...
    "Returns personalized resource and learning approach recommendations based on the student's learning style, the specific career path they're pursuing, and their current difficulty level. Includes preferred resource types, specific platform suggestions, and study strategy advice.",
    {
...
function getLearningApproach(style: string, tech: string): string {
...
function getStudyStrategy(style: string, difficulty: string): string[] {
