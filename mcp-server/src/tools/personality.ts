...
import { hollandTypes, describeHollandCombo } from "../data/holland-map.js";
...
export function registerPersonalityTools(server: McpServer): void {
  ...
  server.tool(
    "get_personality_profile",
    "Returns a comprehensive psychological profile for a given MBTI type and Holland code, including cognitive function stack, strengths, blind spots, ideal work environment, communication style, motivational triggers, stress response, career values, and how the Holland code modifies the profile.",
    {
  ...
  server.tool(
    "get_psychological_nudge",
    "Returns communication framing advice for how to phrase career guidance to a student with a specific MBTI type. Includes the personality's communication style, motivational triggers, things to avoid, and a sample framing for the given context.",
    {
      mbtiType: z.string().length(4).describe("Four-letter MBTI type, e.g. 'INTJ'"),
